...
**Current release tag:** `v1.5.1` (see root `CHANGELOG.md`).
...
```bash
# Pinned v1.5.1 (recommended wrapper)
curl -fsSL https://raw.githubusercontent.com/mazh-cp/CP_Migration_Tool/v1.5.1/deploy/upgrade-production.sh | sudo bash -s -- v1.5.1
```

```bash
# Same, via env
curl -fsSL https://raw.githubusercontent.com/mazh-cp/CP_Migration_Tool/v1.5.1/deploy/upgrade-production.sh | sudo env BRANCH=v1.5.1 bash
```

```bash
# Self-contained updater (no wrapper; same as above for any tag)
curl -fsSL https://raw.githubusercontent.com/mazh-cp/CP_Migration_Tool/v1.5.1/deploy/update_azure_ubuntu.sh | sudo env BRANCH=v1.5.1 bash
```
...
```bash
REMOTE=ubuntu@YOUR-VM ./deploy/upgrade-remote-production.sh v1.5.1
```

...
**Pinned release `v1.6.1` (recommended):**
...
**Pinned release (v1.6.1):** `sudo` does **not** keep `BRANCH` from `BRANCH=v1.x curl | sudo bash`. Pass the tag **inside** `sudo` or use **`bash -s -- <tag>`**:
...
```bash
curl -fsSL https://raw.githubusercontent.com/mazh-cp/CP_Migration_Tool/v1.6.1/deploy/upgrade-production.sh | sudo env BRANCH=v1.6.1 bash
```

**Direct updater** (no wrapper; works for any tag):

```bash
curl -fsSL https://raw.githubusercontent.com/mazh-cp/CP_Migration_Tool/v1.6.1/deploy/update_azure_ubuntu.sh | sudo env BRANCH=v1.6.1 bash
```

Optional: `PORT=3000` for the unit file (health checks also read `PORT` from `apps/web/.env`). Doc banner: `DOC_RELEASE_TAG=v1.6.1`. Builds use `NODE_OPTIONS=--max-old-space-size=6144` by default on the server (override if needed).
